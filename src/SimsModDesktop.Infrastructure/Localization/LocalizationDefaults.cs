namespace SimsModDesktop.Infrastructure.Localization;

internal static class LocalizationDefaults
{
    public static readonly IReadOnlyDictionary<string, string> English =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["status.ready"] = "Ready.",
            ["status.scriptNotFound"] = "Script not found: {0}",
            ["status.validationFailed"] = "Validation failed.",
            ["validation.notStarted"] = "Validation has not started yet.",
            ["validation.failed"] = "Validation failed: {0}",
            ["validation.okToolkit"] = "Validation passed: {0} parameters are valid.",
            ["validation.okTrayPreview"] = "Validation passed: Tray Preview parameters are valid.",
            ["status.unsupportedBrowseTarget"] = "Unsupported {0} browse target: {1}",
            ["status.dangerCancelled"] = "Dangerous operation cancelled.",
            ["status.executionAlreadyRunning"] = "Execution is already running.",
            ["status.running"] = "Running...",
            ["status.executionCompleted"] = "Completed in {0}.",
            ["status.executionFailedExit"] = "Failed with exit code {0} in {1}.",
            ["status.executionCancelled"] = "Execution cancelled.",
            ["status.executionFailed"] = "Execution failed.",
            ["status.unknownExecutionError"] = "Unknown execution error.",
            ["status.trayPreviewLoading"] = "Loading tray preview...",
            ["status.trayPreviewLoaded"] = "Tray preview loaded: {0} items, page 1/{1}, {2}.",
            ["status.trayPreviewCancelled"] = "Tray preview cancelled.",
            ["status.trayPreviewFailed"] = "Tray preview failed.",
            ["status.unknownTrayPreviewError"] = "Unknown tray preview error.",
            ["status.trayPageLoadingAlready"] = "Tray preview page load is already running.",
            ["status.trayPageLoaded"] = "Tray preview page {0}/{1}.",
            ["status.trayPageCancelled"] = "Tray preview page load cancelled.",
            ["status.trayPageFailed"] = "Tray preview page load failed.",
            ["status.unknownTrayPreviewPageError"] = "Unknown tray preview page error.",
            ["status.noRunningExecution"] = "No running execution.",
            ["status.cancelling"] = "Cancelling...",
            ["progress.starting"] = "Starting...",
            ["progress.idle"] = "Idle",
            ["progress.completed"] = "Completed.",
            ["progress.failed"] = "Failed.",
            ["progress.cancelled"] = "Cancelled.",
            ["progress.executionFailed"] = "Execution failed.",
            ["progress.trayLoaded"] = "Tray preview loaded.",
            ["progress.trayFailed"] = "Tray preview failed.",
            ["progress.loadingTray"] = "Loading tray preview...",
            ["preview.noneLoaded"] = "No preview data loaded.",
            ["preview.page"] = "Page {0}/{1}",
            ["preview.lazyCache"] = "Lazy cache {0}/{1} pages",
            ["preview.typeNa"] = "Type: n/a",
            ["preview.type"] = "Type: {0}",
            ["preview.summaryReady"] = "Preview ready. {0}",
            ["preview.range"] = "Showing {0}-{1} / {2} tray presets.",
            ["preview.empty.initial.title"] = "Ready to load tray presets",
            ["preview.empty.initial.description"] = "Click \"Load Preview\" to scan the current Tray folder and build a browsable preset list.",
            ["preview.empty.pathMissing.title"] = "Tray path is not configured",
            ["preview.empty.pathMissing.description"] = "Set a valid Tray Path in Settings first, then return here to load preview data.",
            ["preview.empty.noResults.title"] = "No presets match current filters",
            ["preview.empty.noResults.description"] = "Clear filters or adjust author/time/preset type to widen the result scope.",
            ["preview.empty.status.ready"] = "Ready to load",
            ["preview.empty.status.pathMissing"] = "Path missing",
            ["preview.empty.status.noResults"] = "No matching results",
            ["dialog.danger.title"] = "Confirm Dangerous Action",
            ["dialog.danger.message"] = "You are about to run FindDuplicates cleanup mode; duplicate files will be deleted (first path kept).",
            ["dialog.danger.confirm"] = "Run Anyway",
            ["dialog.danger.cancel"] = "Cancel",
            ["dialog.danger.note"] = "This action may delete files and cannot be undone.",
            ["dialog.confirm.continue"] = "Please confirm to continue.",
            ["dialog.error.title"] = "Operation Failed",
            ["dialog.error.confirm"] = "OK",
            ["dialog.error.checkLog"] = "See logs for details.",
            ["ui.settings.title"] = "Option Settings",
            ["ui.settings.section.lowFrequency"] = "Low-frequency Advanced Tuning",
            ["ui.settings.section.lowFrequencyHint"] = "High-frequency options are now shown on each feature page; only low-frequency tuning stays here.",
            ["ui.settings.language.label"] = "UI Language",
            ["ui.settings.language.todoHint"] = "Chinese translation is TODO. Current Chinese resources are placeholders.",
            ["ui.settings.prefixHashBytes.label"] = "Prefix Hash Bytes",
            ["ui.settings.hashWorkerCount.label"] = "Hash Worker Count",
            ["ui.settings.performance.hint"] = "Adjust these only for large directories or performance tuning scenarios."
        };
}

